...
<task type="auto">
  <name>Task 1: Create preferencesService for localStorage abstraction</name>
...
**Service methods:**
1. `load(): UserPreferences` - Read from localStorage, parse JSON, validate structure, merge with defaults for missing keys. On parse error or invalid structure: log error, show warning toast via sonner ("Preferences corrupted, reset to defaults"), return defaults.

2. `save(preferences: UserPreferences): void` - Stringify and write to localStorage. On error (quota exceeded, etc.): log error, show error toast via sonner ("Failed to save preferences").
...
Run: `npx tsc --noEmit -p frontend` to verify TypeScript compiles without errors.
  ...
  <done>
preferencesService exports load(), save(), getDefaults() methods and UserPreferences type. All localStorage access is wrapped in try-catch with toast notifications on error.
  </done>
</task>

<task type="auto">
  <name>Task 2: Create PreferencesContext with persistence and cross-tab sync</name>
...
<task type="auto">
  <name>Task 3: Wire PreferencesProvider into App.tsx</name>
...
5. No TypeScript errors, no console errors on app load
</success_criteria>
