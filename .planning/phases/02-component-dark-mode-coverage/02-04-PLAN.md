...
must_haves:
  truths:
    ...
    - "Preferences page text has proper contrast"
  artifacts:
...
<objective>
Fix Teachers, Schools, Settings, and PreferencesSettings pages for dark mode by replacing hardcoded colors with semantic tokens.
...
Output: All main application pages render correctly in both light and dark themes.
</objective>
...
<task type="auto">
  <name>Task 1: Update Teachers page with semantic colors</name>
  <files>frontend/src/pages/Teachers.tsx</files>
  <action>
Replace hardcoded colors in Teachers.tsx:
...
`grep -E "text-slate-[0-9]" frontend/src/pages/Teachers.tsx` should return empty.
  </verify>
  <done>
Teachers page uses text-foreground for headings, text-muted-foreground for subtitles and icons.
  </done>
</task>

<task type="auto">
  <name>Task 2: Update Schools page with semantic colors</name>
  <files>frontend/src/pages/Schools.tsx</files>
  <action>
Replace hardcoded colors in Schools.tsx:
...
- `bg-red-600 hover:bg-red-700` -> Keep (destructive)
  ...
  <verify>
`npm run build --prefix frontend` passes.
`grep -E "bg-white|text-slate-[0-9]|border-slate|hover:bg-slate" frontend/src/pages/Schools.tsx` should return empty.
  </verify>
  <done>
Schools page uses bg-card for surfaces, border-border for borders, hover:bg-muted/50 for table rows, text-foreground for headings, text-muted-foreground for secondary text.
  </done>
</task>

<task type="auto">
  <name>Task 3: Update Settings pages with semantic colors</name>
  <files>
    frontend/src/pages/Settings.tsx
    frontend/src/pages/PreferencesSettings.tsx
  </files>
...
Card components use Shadcn defaults which are already semantic.
  ...
  <verify>
`npm run build --prefix frontend` passes.
`grep -E "text-slate-[0-9]" frontend/src/pages/Settings.tsx frontend/src/pages/PreferencesSettings.tsx` should return empty or minimal.
  </verify>
  <done>
...
5. PreferencesSettings.tsx subtitles use text-muted-foreground
</verification>
