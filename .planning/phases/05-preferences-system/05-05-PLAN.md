...
must_haves:
  truths:
    - "Settings page shows clear sections: Appearance, Language, Accessibility"
    - "Global reset button resets ALL preferences to defaults"
    - "Reset confirmation dialog prevents accidental resets"
    - "Preference changes sync across multiple browser tabs in real-time"
    - "All controls work correctly with translations in both languages"
  artifacts:
...
Add imports for the reset confirmation dialog:
```typescript
import {
  AlertDialog,
  AlertDialogAction,
...
"reset": {
  ...
  "dialog": {
    ...
    "description": "This will reset theme, font size, display density, and reduced motion to their default values. This action cannot be undone.",
    "cancel": "Cancel",
    "confirm": "Reset All"
  }
}
...
"reset": {
  ...
  "dialog": {
    "title": "確定要重設所有偏好設定？",
    "description": "這將會重設主題、字體大小、顯示密度和減少動態效果至預設值。此操作無法復原。",
    "cancel": "取消",
    "confirm": "全部重設"
  }
}
...
**Note:** The reset should NOT reset theme (handled by next-themes) or language (handled by i18next). Only reset the preferences managed by PreferencesContext: fontSize, density, reducedMotion.

Actually, per CONTEXT.md: "Global reset button that resets ALL preferences to defaults" - but theme and language are managed by separate systems. The reset button should reset what PreferencesContext manages.
...
The storage event listener should not only update state, but also apply the preferences to DOM (since the DOM-setting effect only runs on state changes from the current tab):
  ...
  const handleStorageChange = (e: StorageEvent) => {
    if (e.key === 'userPreferences' && e.newValue) {
      try {
        ...
        // (The normal useEffect will also run, but this ensures immediate update)
...
**Note:** The state update via setPreferences will also trigger the persistence useEffect, but that's fine - it will just re-save the same values (no-op effectively). The immediate DOM update ensures visual sync happens right away.

**Alternative (cleaner):** Create a helper function that applies preferences to DOM, called from both the persistence effect and the storage event handler.
  </action>
  <verify>
