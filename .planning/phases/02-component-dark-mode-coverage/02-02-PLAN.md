...
<task type="auto">
  <name>Task 3: Update PipelineChart and DemographicsChart with theme-aware styling</name>
  <files>
    frontend/src/components/dashboard/PipelineChart.tsx
    frontend/src/components/dashboard/DemographicsChart.tsx
  </files>
  <action>
Update these chart components to use CSS variables for theming. Import helpers from chartColors.ts.

For BOTH chart files, apply these patterns:
...
Update imports to include getAxisColor, getBorderColor from chartColors.ts if using those helpers.
  ...
  <verify>
Run `npm run build --prefix frontend` to verify no errors.
`grep -rE "fill.*#[0-9a-fA-F]{6}|text-slate-[0-9]" frontend/src/components/dashboard/PipelineChart.tsx frontend/src/components/dashboard/DemographicsChart.tsx` should return minimal/no matches.
  </verify>
  <done>
PipelineChart and DemographicsChart use CSS variables for axis labels, grid lines, tooltips, and label lists.
  </done>
</task>

<task type="auto">
  <name>Task 4: Update EducationChart, SalaryChart, and SeniorityChart with theme-aware styling</name>
  <files>
    frontend/src/components/dashboard/EducationChart.tsx
    frontend/src/components/dashboard/SalaryChart.tsx
    frontend/src/components/dashboard/SeniorityChart.tsx
  ...
  <action>
Apply the same theming patterns from Task 3 to the remaining chart components:
...
5. **CardTitle and text**: Replace `text-slate-*` classes with `text-foreground` or `text-muted-foreground` as appropriate

6. **Background colors**: Replace `bg-slate-*` with `bg-muted` or `bg-card` as appropriate
  </action>
  <verify>
Run `npm run build --prefix frontend` to verify no errors.
`grep -rE "fill.*#[0-9a-fA-F]{6}|text-slate-[0-9]" frontend/src/components/dashboard/EducationChart.tsx frontend/src/components/dashboard/SalaryChart.tsx frontend/src/components/dashboard/SeniorityChart.tsx` should return minimal/no matches.
  </verify>
  <done>
EducationChart, SalaryChart, and SeniorityChart use CSS variables for all chart theming. No hardcoded #64748b hex colors remain.
  </done>
</task>
