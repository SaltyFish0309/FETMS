...
<task type="auto">
  ...
  <action>
Update the useEffect that persists preferences to also set the data-font-size attribute on document.documentElement.

**Modify the persistence useEffect:**

Find the useEffect that calls preferencesService.save() and add DOM attribute setting:

```typescript
useEffect(() => {
  preferencesService.save(preferences);

  // Apply font size to DOM for CSS custom properties
  document.documentElement.setAttribute('data-font-size', preferences.fontSize);
}, [preferences]);
```

**Important:** This sets the attribute on the <html> element, which :root CSS selectors target.

**Also add initialization:** In the PreferencesProvider component, add an effect that runs once on mount to apply initial font size (before the persistence effect runs):

```typescript
// Apply initial preferences to DOM
useEffect(() => {
  document.documentElement.setAttribute('data-font-size', preferences.fontSize);
...
Actually, better approach: The persistence effect already handles this on first render. But to ensure immediate DOM sync, move the setAttribute inside the state initialization or keep in the main effect.

Simplest: Just keep one useEffect that both saves and applies to DOM. On first render, it will apply the loaded preferences.
  </action>
  <verify>
...
Verify font-size CSS applies correctly.
  ...
  <done>
PreferencesContext sets data-font-size attribute on document.documentElement whenever preferences change. Initial load applies default or stored font size.
  </done>
</task>
